...
func httpGet(url int, response chan string, wg *sync.WaitGroup) {
	// 函数执行完毕时 计数器-1
	defer wg.Done()
...
func collect(urls []int) []string {
	...
	for _, url := range urls {
		...
		// go httpGet(url, responseChannel, wg)
		go func() {
			defer wg.Done()
			// limiter 只能接受指定数量的数据，否则就等待,chan天生为并发而生
			limiter <- true
			time.Sleep(300 * time.Millisecond)
			responseChannel <- fmt.Sprintf("http get: %d", url)
			<-limiter
		}()
	}
	// 启动读取结果的控制器
